...
%           robut (1 default or 0) indicate the type of estimator to use ie
%           trimmed mean and winsorized variance or mean and variance
%
...
switch type
    ...
    case{1}  % 1 factor
        ...
        if robust == 1
            S         = rst_wincov(Data);    % robust covariance to account for spericity
            y         = rst_trimmean(Data)'; % these are the trimmed means to compare
            g         = floor((20/100)*n);   % number of items to winsorize and trim
            h         = n-2.*g;              % effective sample size after trimming
            df        = p;                   % instead of p-1;
            dfe       = h-p;                 % adjusted dfe
            dof_ratio = dfe / ((h-1)*df);    
            Tsquare   = h*(C*y)'*inv(C*S*C')*(C*y); % robust Hotelling Tsquare
            result.T2 = Tsquare;
            result.F  = dof_ratio * Tsquare;
            result.df = [df dfe];
            result.p  = 1 - fcdf(result.F, df, dfe);
        else
            S         = cov(Data);                 % covariance to account for spericity
            y         = nanmean(Data,1)';          % these are the trimmed means to compare
            df        = p-1;                       % number of measures - 1
            dfe       = n-p+1;                     % nb of observations - measures
        ...
        if nboot > 0
            ...
            for B=1:nboot
                ...
                if robust == 1
                    S          = rst_wincov(boot_Data); 
                    y          = rst_trimmean(boot_Data)'; 
                    Tsquare(B) = h*(C*y)'*inv(C*S*C')*(C*y);
                    F(B)       = dof_ratio * Tsquare(B);
                else
                    S          = cov(boot_Data);
                    y          = nanmean(boot_Data,1)';
                    Tsquare(B) = n*(C*y)'*inv(C*S*C')*(C*y);
                    F(B)       = dof_ratio * Tsquare(B);
                end
            end
        end
        ...
        % get p value
        T = sort(Tsquare); 
        result.pboot = 1-((sum(result.T2 > Tsquare))/nboot); % = how many times the observed F value > F under H0;
        if result.pboot == 0
            result.pboot = 1/nboot;
        end
        ...
        if robust == 1
            ...
            if iscell(C)
                for effect = 1:size(C,2)
                    c                 = C{effect};
                    df(effect)        = rank(c)+1; % size(c,1);
                    dfe(effect)       = h-rank(c);
                    interaction_y     = rst_trimmean([c*Data']')';
                    Tsquare           = h*(interaction_y)'*inv(c*S*c')*(interaction_y); 
                    result.T2(effect) = Tsquare;
                    result.F(effect)  = ( dfe(effect) / ((h-1)*(df(effect))) ) * Tsquare;
                end
                
%                 % main effects
%                 for effect = 1:length(factors)
%                     c                 = C{effect};
%                     df(effect)        = rank(c); % size(c,1);
%                     dfe(effect)       = h-df(effect);
%                     Tsquare           = h*(c*y)'*inv(c*S*c')*(c*y); 
%                     Tsquare           = h*(interaction_y)'*inv(c*S*c')*(interaction_y); 
%                     result.T2(effect) = Tsquare;
%                     result.F(effect)  = ( dfe(effect) / ((h-1)*(df(effect))) ) * Tsquare;
%                 end
%                 
%                 % interactions
%                 for inter_level = 2:length(factors)
%                     Combs  = nchoosek([1:length(factors)],inter_level);
%                     for e = (effect+1):(size(Combs,1)+effect)
%                         df(e) = prod(df(Combs(e-effect,:)));
%                         dfe(e) = h-rank(C{effect});
%                     end
%                     effect = effect+size(Combs,1);
%                 end
%                 
%                 for effect = length(factors)+1:size(C,2)
%                     c                 = C{effect};
%                     % dfe(effect)       = h-df(effect);
%                     interaction_y     = rst_trimmean([c*Data']')';
%                     Tsquare           = h*(interaction_y)'*inv(c*S*c')*(interaction_y); 
...
%                 end
        ...
        if nboot > 0
            ...
            for B=1:nboot
                ...
                boot_Data = Data_centered(table(:,B),:);
                if robust == 1
                    S = rst_wincov(boot_Data); % covariance to account for spericity
                    y = rst_trimmean(boot_Data)'; % these are the means to compare
             
                    if iscell(C)
                        for effect = 1:size(C,2)
                            c = C{effect};
                            Tsquare(effect,B) = h*(c*y)'*inv(c*S*c')*(c*y); % is also t = sqrt(n*(c*y)'*inv(c*S*c')*(c*y));
                            F(effect,B) = ( dfe(effect) / ((h-1)*(df(effect))) ) * Tsquare(effect,B);
                        end
                    else
                        Tsquare(B) = h*(C*y)'*inv(C*S*C')*(C*y);
                        F(B) = ( dfe / ((h-1)*df) ) * Tsquare(B);
                    end
                else
                    S = nancov(boot_Data);
                    y = nanmean(boot_Data,1)';
                    ...
                    if iscell(C)
                        for effect = 1:size(C,2)
                            c = C{effect};
                            Tsquare(effect,B) = n*(c*y)'*inv(c*S*c')*(c*y); % is also t = sqrt(n*(c*y)'*inv(c*S*c')*(c*y));
                            F(effect,B) = ( dfe(effect) / ((n-1)*(df(effect))) ) * Tsquare(effect,B);
                        end
                    else
                        Tsquare(B) = n*(C*y)'*inv(C*S*C')*(C*y);
                        F(B) = ( dfe / ((n-1)*df) ) * Tsquare(B);
                    end
                end
            end % closes the bootstrap loop
            
            % get p values
            if iscell(C)
                for e = 1:nb_effects
                    T = sort(Tsquare(e,:));
                    result.pboot(e) = 1-(sum(result.T2(e)>Tsquare(e,:))/nboot);
                    if result.pboot(e) == 0
                        result.pboot(e) = 1/nboot;
                    end
                end
            else
                T = sort(Tsquare);
