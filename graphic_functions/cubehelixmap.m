function map = cubehelixmap(varargin)

% FORMAT: map = cubexhelixmap(type,value)
%
% INPUT: cubehelixmap('increase',size)
%        cubehelixmap('increase_lowbound',value,size)
%        cubehelixmap('decrease',size)
%        cubehelixmap('decrease_highbound',value,size)
%        cubehelixmap('continuous,size')
%        cubehelixmap('semi_continuous',size)
%
% size is the size of your map
% value is the bound of your data, mapped onto a 0 to 1 scale
% 
% OUTPUT: map is a 3 channels colormap
%
% The schema follows roughly Christen et al. 2013 NeuroImage 73 p30-39
% The colours however vary with perceived intensity based on the
% cubehelix: Green, D. A., 2011, `A colour scheme for the display of
% astronomical intensity images', Bulletin of the Astronomical 
% Society of India, 39, 289. 
%
% The cubehelix code is a copy of Stephen Cobeldick's code
%----------------------------------------------------------------------
...
if strcmpi(varargin{1},'increase')
    if nargin == 1
        varargin{2} = 64;
    end
    map = cubehelix(varargin{2},[1.15,0.1,4,1], [0,1], [0,0.85]);
    
elseif strcmpi(varargin{1},'increase_lowbound')
    if nargin == 1
        error('you have to input the low bound')
    elseif nargin == 2
        varargin{3} = 64;
    end
    map = cubehelix(varargin{3},[1.15,0.1,4,1], [0,1], [varargin{2},0.85]);

elseif strcmpi(varargin{1},'decrease')
    if nargin == 1
        varargin{2} = 64;
    end
    map = cubehelix(varargin{2},[1.8,0.3,4,1], [0.2,1], [0.2,0.85]);

elseif strcmpi(varargin{1},'decrease_highbound')
    if nargin == 1
        error('you have to input the high bound')
    elseif nargin == 2
        varargin{3} = 64;
    end
    map = cubehelix(varargin{3},[1.8,0.3,4,1], [0.2,1], [0.2+varargin{2},0.85]);
    
elseif strcmpi(varargin{1},'continuous')
    if nargin == 1
        varargin{2} = 64;
    end
    map = cubehelix(varargin{2},[3,0.25,3,1], [0,1], [0.1,0.85]);
    
elseif strcmpi(varargin{1},'semi_continuous')
    if nargin == 1
        varargin{2} = 64;
    end
    map = cubehelix(varargin{2},[3,1.9,1.5,1], [0,1], [0.2,0.8]);
